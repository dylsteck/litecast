...
import { removeStoredSigner } from '../lib/farcaster/signer';
...
const HomeHeaderRight = () => {
  ...
  const isSelected = (name: string) => {
    ...
    const params = currentRoute.params as
      | { type?: string; parent_url?: string }
      | undefined;
    if (name === 'index' && currentRoute.name === 'index') return true;
    if (name === 'trending' && currentRoute.name === 'channel' && params?.type === 'trending')
      return true;
    if (
      name === DEV_CHANNEL_URL &&
      currentRoute.name === 'channel' &&
      params?.parent_url === DEV_CHANNEL_URL
    )
      return true;
    if (
      name === PURPLE_CHANNEL_URL &&
      currentRoute.name === 'channel' &&
      params?.parent_url === PURPLE_CHANNEL_URL
    )
      return true;
    return false;
  };
